...
// Connect to network device with device-specific handling
...
func (lib *NetworkLibrary) ExecuteCommand(handle, command string) (string, error) {
	...
	// IOS-XR requires PTY
	if strings.Contains(conn.deviceType, "ASR99") || strings.Contains(conn.deviceType, "IOS-XR") {
	...
	if err != nil {
		...
		log.Printf("Command completed with status: %v", err)
	}
...
func parseOSPFNeighbors(output string) []map[string]string {
	...
	for _, line := range lines {
		// Look for neighbor entries (contains IP addresses and FULL state)
...
func parseBGPSummary(output string) map[string]interface{} {
	...
	for _, line := range lines {
		// Look for peer entries (IP address followed by ASN)
...
func (lib *NetworkLibrary) CloseConnection(handle string) error {
	...
	log.Printf("✓ Closed connection to %s", conn.hostname)
	return err
}
