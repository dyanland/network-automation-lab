...
func (lib *NetworkLibrary) ConnectToDevice(hostname, deviceType, username, password string) (string, error) {
	...
	// Store connection info (will reconnect for each command)
	conn := &SSHConnection{
		hostname:   hostname,
...
// Execute command - creates fresh session each time
func (lib *NetworkLibrary) ExecuteCommand(handle, command string) (string, error) {
	...
	client, err := ssh.Dial("tcp", conn.hostname+":22", conn.config)
	if err != nil {
